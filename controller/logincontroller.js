...
const otpgenarating = async (req, res) => {
  ...
  if (findnumber) {
    return res
      ...
      .json({
        status: true,
        message: "OTP sent and number added to the database",
        data: findnumber,
...
const otpverification = async (req, res) => {
  ...
  if (response.valid) {
    ...
    if (!findusercontact) {
      ...
      const usertoken = jwt.sign(
        {
          id: userse._id,
          number: userse.number,
          contactid: savecollection._id,
        },
  ...
  sameSite: 'none',
      ...
      res
        .status(200)
        .json({
          message: "Welcome verfication sucsses",
          data: response,
          token: usertoken,
          usercontact: savecollection,
        });
    }
    ...
    console.log("userse", userse._id);
    const usertoken = jwt.sign(
      { id: userse._id, number: userse.number, contactid: findusercontact._id },
      process.env.JWT_SECRET_KEY,
      { expiresIn: "30d" }
    );
...
const adduserdetails = async (req, res) => {
  ...
  res
    ...
    .json({
      status: true,
      message: "user details sucssesfully updated",
      data: upadteuser,
    });
};
...
const getspacificuser = async (req, res) => {
  ...
  res
    ...
    .json({
      status: true,
      message: "get spacifc user profile details",
      data: userse,
    });
};
...
const savecontacts = async (req, res) => {
  ...
  if (!findenumberuser) {
    return res
      ...
      .json({
        status: false,
        message: "this numbe not have a whatsapp acoount pleas invte",
        number: number,
      });
  }
  ...
  if (findcontact) {
    return res
      ...
      .json({
        status: false,
        message: "this number already you saved your system",
        number: number,
      });
  }
  ...
  return res
    .status(200)
    .json({ status: true, message: "number saved.", data: savecollection });
};

/////////////////////////////// GET ALL CONTACTS ////////////////////////

const getallcontacts = async (req, res) => {
  const contatctes = await contact
    .find({ userid: req.user.id })
    .populate("profileimage", "profileimage _id");
  // console.log("mesage",contatctes);
  const filtercontact = contatctes.filter(
    (item) => item.number !== req.user.number
  );

  res
    .status(200)
    .json({ status: true, message: "get all contatcs", data: filtercontact });
};
...
const updateprofile = async (req, res) => {
  ...
  io.emit("updated_profile",savefinduser);

  res
    .status(200)
    .json({ status: true, message: "profile updated", data: savefinduser });
};
